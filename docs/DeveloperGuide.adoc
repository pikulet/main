...
there are 100 rooms. The `RoomNotFoundException` is not unexpected to occur
under any circumstances of user input, but is left there as a defensive measure.
...
field. However, it is troublesome to enter a `BookingPeriod` manually, as it entails
...
are outdated but not expired, since these bookings can no longer be added.
...
particular `Room` can be put together, allowing the Receptionist to determine
 ...
 `Booking` s in a room. In Concierge, a `Room` is unlikely to have a large number
 of advanced bookings made.
** Cons: Difficult to find the `Room` given the `Guest`. When a `Guest` has made
an advanced booking and wishes to cancel it, we have to search through all the `Room` s.
However, this is essentially an `O(1)` time operation since the number of `Room` s is constant.
 *** In fact, for most uses of Concierge, we expect `Guest` s to be aware of
...
*** As of v1.4, our implementation includes the list of rooms, and two auxiliary lists:
checked-in guests and archived guests. These lists help the receptionist to look up the personal
particulars of the `Guest` quickly. Because `Booking` information is not needed when a receptionist
intends to look up guest particulars, these lists do not retain any history on the bookings.
// end::add[]
...
algorithm can be changed to a HMAC hash which adds a username salt instead.

More information on the design consideration for this aspect can be found
<<Aspect: Storage of Passwords, here>>.
...
Some of the more commonly used Concierge features (`list`, `find`) are read-only features.
...
call to a method (e.g. `performLoginChecks()`) in the superclass. The
proposed solution to this would be to implement the login checks in the `Command#execute`
method Every command must call `super()` in its respective `execute` method to perform
the login checks.
*** While increasing cohesion, this implementation makes less semantic sense.
