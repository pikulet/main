...
image::AddCommand-activity-diagram.png[width="500"]
...
the `Guest`, include `Booking` s made and `Expense` s incurred.
...
checked-in guests. This list does not retain any `Booking` information, as it
 is meant to help the receptionist quickly look up the `Guest` 's particulars.
// end::add[]
