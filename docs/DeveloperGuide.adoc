...
** Test case: `checkin r/010` +
Expected: Cannot check-in room 010 as first booking is already expired.
...
** Prerequisites: There is a booking in the room you want to check out. Room 002 has a preloaded non-checked-in booking for you to test.
** Test case: `checkout r/002` +
...
** Prerequisites: There is a booking in the room you want to check out. Room 001 has a preloaded checked-in booking for you to test.
** Test case: `checkout r/001` +
Expected: First booking is deleted. Guest Alex Yeoh is removed from the checked-in guest list and added to the archived guest list.

* Checking out specified booking of a room that is not checked-in
** Prerequisites: There is a booking in the room you want to check out. Room 003 has a preloaded upcoming booking for you to test.
** Test case: `checkout r/003 from/14/11/18` +
Expected: Booking is deleted. Guest of booking is not added into checked-in guest list nor archived guest list.

=== Reassign

First, ensure that you have signed in by doing `login user/admin pw/passw0rd`.

* Reassigning a non-checked-in booking to another
** Prerequisites: There is a booking in the room you want to reassign. Room 004 has a preloaded booking for you to test.
** Test case: `reassign r/004 from/12/11/18 nr/007` +
Expected: Booking in room 004 that starts from 12/11/18 is reassigned to room 007 (which has no bookings prior).

* Incorrect commands to try:
** `reassign r/001 from/32/11/18 nr/002` (no such date)
** `reassign r/005 from/12/11/18 nr/101` (no such rooms)
