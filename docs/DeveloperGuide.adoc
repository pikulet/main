...
managed by the `JsonPasswordsStorage` class, which interacts with the main
Concierge software via the `PasswordsStorage` interface as encouraged by the
Dependency Inversion Principle. Intuitively, passwords are stored as
