...
An example of its usage: `add n/Madith p/83141592 e/madith@themyth.com a/Recluse Street -r 041 from/
...
Currently, the `login` is implemented as a dynamic log-in, so users are not
...
Additionally, a new method `resetUndoRedoHistory` was added to the
...
The `resetUndoRedoHistory` first sets the `currentStatePointer` to 0, then
commits the current `VersionedConcierge`.

More details on the `VersionedConcierge` can be
understood from the <<Undo/Redo feature, undo and redo documentation>>.
...
===== Checking if commands require sign-in

`LogicManager` checks if commands require sign-in.
...
** Pros:
*** Allows manager to implement some level of access control
*** Quickly start a new session with a new user.
** Cons: Not intuitive to users. They have to enter commands before being told
they need to sign in.

TODO: explain the use of requiresSignIn

* **Alternative 2:** Login is needed for all features
** Pros: Very easy check when Concierge is first loaded
** Cons: Logout would invoke a new window/ close the session.
...
** Cons: Exposes implementation of password hashing. Prone to error.
...
** Pros:
*** Able to make use of existing `JsonUtil` methods used by the
`UserPrefs` and `Config` classes.
*** Able to parse data easily into key-value pairs

* **Alternative 2:** Store passwords in same XML file as all other
Concierge data.
** Pros:
*** Centralises data storage in Concierge.
*** Concierge only performs one read operation for the data.
** Cons:
*** User is overloaded with information from XML (but they should be familiar)
*** Creating a new entry is very difficult. (But new entries not created often)
