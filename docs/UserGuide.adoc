...
// tag::add[]
=== Adding a guest: `add` image:icon-key.png[width="32"]
...
Adding an active booking (i.e. booking starts today):

image::AddCommand-userguide-active.png[width="800"]

You can preview the booking on the left room pane. +

Adding an inactive booking (i.e. booking is in the future): you can view the
booking by selecting the relevant room, under "All other bookings".
...
<<Adding a guest: `add` image:icon-key.png[width="32"], `add`>>,
<<Check-in room: `checkin` image:icon-key.png[width="32"], `checkin`>>,
<<Checkout room: `checkout` image:icon-key.png[width="32"], `checkout`>>,
<<Reassigning a guest to another room : `reassign` image:icon-key.png[width="32"],`reassign`>>,
<<Provide Room Service: `service` image:icon-key.png[width="32"], `service`>>,
<<Clearing all entries : `clear` image:icon-key.png[width="32"], `clear`>>
...
==== Adding a new account
 ...
 Currently, Concierge(TM) does not have a feature for users to add an account
  via the app. The reason is because we only read from the password file. Nevertheless, for the adventurous users who want to create their own accounts, this sub-section will be useful.

Step 1: Open the `passwords.json` file, which should be in the same
folder as your `concierge.jar` file. This file stores a list of hashed
passwords.

Step 2: Concierge(TM) uses SHA-256 password hashing. This hash can be
...
[NOTE]
 Concierge(TM) was designed to work with alphanumeric usernames and passwords in
...
Step 4: Close and reload the Concierge(TM) application, and your new account
will be recognised. You are now able to sign in with the new account.

image::LogInCommand-newloginsuccess.png[width="400"]
...
** This achieves the same effect as closing and re-opening Concierge(TM) in the
