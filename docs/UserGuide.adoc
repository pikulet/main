...
Commands which require login: <<Adding a guest: `add`, `add`>>, <<Check-in room: `checkin`, `checkin`>>,
<<Checkout room: `checkout`, `checkout`>>, <<Reassigning a guest to another room : `reassign`, `reassign`>>,
<<Provide Room Service: `service`, `service`>>, <<Clearing all entries : `clear`, `clear`>>
...
After sign in, the `clear` command can now be executed.
...
==== Adding a new account
 ...
 Currently, Concierge(TM) does not have a feature for users to add an account
  via the app. The reason is because we only read from the password file.
...
folder as your Concierge.jar file. This file stores a list of hashed passwords.
